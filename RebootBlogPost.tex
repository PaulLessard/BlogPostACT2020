...
\paragraph{Concise} When stripped of the philosophical premises of what terms, types, judgements etc. are, the rules of a type theory can be seen to generate its derivable judgments. It is therefore desireable that the translation between symmetric categories and the rules of their associated type theories procede by way of the most concise combinatorial description of symmetric monoidal categories possible.
...
in which:
\begin{itemize}
	\item {\color{blue} $\Gamma$ is a context, and $\Delta$ is a (list of) type(s).}
...
Provided we are careful however - meaning we do not perform any "illegal moves" in a sense we'll make clear - we can still use lists; tis is what Shulman does in adapting Sweedler's notation. Consider a general element
%
    \[
...
\begin{remark}
Taking a hint from the bi-categorical playbook can give us a more geometric picture for the difference between an indecomposeable tuple and a decomposeable one is. Since every symmetric monoidal category is also a bi-category with a single object, so typed terms correspond FINISH THIS
\end{remark}
