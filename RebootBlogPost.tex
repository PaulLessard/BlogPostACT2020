...
Many well-known type theories, for example Martin-L\"{o}f dependent type theory or linear type theory, were developed with the aim of syntactically treating a particular form of reasoning; constructive mathematical reasoning in the case of Martin-L\"{o}f type theory and resource dependent computation in the case of linear type theory. It then just also happens that these provide a convenient mean to reason about locally Cartesian closed categories or $\star$-autonomous categories.

In this post, as part of the Applied Category Theory Adjoint School (2020), we will discuss a recent-ish paper by Mike Shulman, 
\textit{A Practical Type Theory for Symmetric Monoidal Categories}, where the author reverses this approach. 
That it, he starts with symmetric monoidal categories as the intended semantics and then (reverse)-engineers a syntax in which it is \emph{practical} to reason about such categories.

This of course brings the question of which properties of a type theory make it practical. Shulman, synthesizing various observations, settles on a few basic tenets to guide the invention of the syntax (and it's interpretation into symmetric monoidal categories); we reduce these further to the conditions that the type theory be: (1) intuitive, (2) concise, and (3) effective.
...
\paragraph{Concise} Second, when working with a specific category, one tends, whether explicitly acknowledged or not, to work with a presentation of the category in terms of generators and relations. For example, think of the simplicial category $\bigtriangleup$ generated by face and degeneracy maps subject to the simplicial identities. As such, our type theory should not be directly specified by a symmetric monoidal category, but rather by a concise presentation of it. More precisely even, we will use a presentation of \textsf{PROP}s.
...
a \textsf{PROP}. This is not however too hard to see: the equivalence between \textsf{PROP}s and symmetric monoidal categories simply "unwinds" {\color{red} what does unwind refer too exactly?} every equality of objects $A\otimes B = C$ into an isomorphism $A \otimes B \overset{\sim}{\longrightarrow} C$, rendering free the monoidal product. This might not make mush sense yet, but the following examples should help you understand \textsf{PROP}s better.
...
\begin{example} {\color{red} Define notation $\mathsf{List}(\mathbf{X}_0)^2$****}
...
It is from these presentations $(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)$ of $\mathsf{PROP}s$ that we will build type theories $\mathsf{PTT}_{(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)}$ for the symmetric monoidal categories $F(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)$. Indeed, reading $\rightsquigarrow$ as ``gives rise to'', we will see the following:
    \begin{eqnarray*}
      \mathbf{Generators}
        &
            &
...
Typing judgments, as promised, correspond to morphisms in $F(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)$. Here are some examples of this correspondence:
%
% \begin{eqnarray*}
...
in which:
\begin{itemize}
	\item {\color{blue} $\Gamma$ is a context, while $\Delta$ is a list of types. ???}
...
%
%
While we don't always think about it, we write contexts in Cartesian type theories as lists of typed variables because of the universal property of the product - a universal property characterized in terms of projection maps. That is, in a Cartesian type theory lists can be recovered from the list of their projections. This is not the case for arbitrary symmetric monoidal products and in general there are no projection maps and even when there are, there is no guarantee that they have a similar computation rule. {\color{red} what similar computation rule?}
...
To illustrate this in a mathematical context, consider a pair of vector spaces $U_1$ and $U_2$. Any element $z\in U_1 \times U_2$ of the Cartesian product of $U_1$ and $U_2$ is uniquely specified by the pair of elements $\mathsf{pr}_{1}(z) \in U_1$ and $\msf{pr}_{2}(z)\in U_2$. However, elements of the tensor product $U_1 \otimes U_2$ need not be simple tensors of the form $x \otimes y$ and, instead, are generally linear combinations $\sum_{i=1}^{k}x_{1,i} \otimes x_{2,i}$ of simple tensors.

Provided we are careful however, we can still use lists - this is what Shulman does by using, Sweedler's notation. Consider a general element
%
    \[
        \sum_{i=1}^{k}x_{1,i}\otimes x_{2,i} \in U_1 \otimes U_2
    \]
In Sweedler's notation this is written as $  (x_{(1)}, x_{(2)})$ with the summation, indices, and tensor symbols all dropped. Provided we make sure that any expression in which $(x,y)$ appears is linear in both variables, this seeming recklessness introduces no errors. This practice is what Shulman develops into the core of his practical type theory.
...
\]
%
Now, as our rules which derive typing judgments come from the generating arrows in the set $\mathbf{X}_1$ unless we find some $p:(B_1,\dots,B_n) \rightarrow B_k$ in $\mathbf{X}_1$ which acts like a projection, a property encoded by relations in $\mathbf{R}$, we simply cannot derive a ``sub-judgment'' $a:A \dashv f_{(k)}:B_k$ for some $k$ between $1$ and $n$. %Unless our presentation provides us with projection maps, we do not have access to them.
...
Let's now look at what the rules of the type theory for this presentation $(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)$. While we haven't bothered with them until now, Shulman's practical type theory does have rules for term formation. Let's have a quick look at a streamlined version of the rules for the term judgments in the type theory for the free dual pair. There are three such rules:
%
%In the case of the practical type theory for the free dual pair, the rules for the term judgment are few. We present a streamlined version of them:
%
\begin{align}
...
\end{align}
The first rule is the usual variable rule giving us the terms in which we'll be able to derive the typing judgments corresponding to identities. The second one gives us terms in which we'll be able to derive co-evaluation as a typing judgment, while the last one gives us the terms with which we'll be able to derive evaluation as a typing judgment.
%
%\begin{itemize}
...
\begin{itemize}
    \item the rule
        ...
        \]
        corresponds to applying a tensor product followed by a braiding $\sigma$ and a shuffling of scalars. The tensor product is taken between generating scalar-valued functions $h,\dots,k$ (which can only be some number of copies of $\evmap$ in our case) and the identity on some object, here $\overrightarrow{E}$.
 %       
 %       
%        of:
%        \begin{itemize}
...
%        \end{itemize} and
        \item the rule
            ...
            \]
            corresponds to applying a braiding $\sigma$ to the tensoring of some number of generating constants, here $f,\dots,g$ (which can only be some number of copies of $\coev$), and the identity on some object $\overrightarrow{A}$.
%            \begin{itemize}
...
\begin{remark} Here we are ignoring the notion consideration of ``activeness'' (a technical device introduced to rigidify the type theory into one with unique derivations (of judgments)). We also ignore some additional syntactic sugar which acts like formal variables for the unit type.
\end{remark}
%
%
...
%
 %
        ...
        \]
    together {\color{red} with rules enough to generate a congruence.  What????}

\begin{example} For a good example, let's look at the derivation of the composition
...
\]
{\color{red} explain this more maybe?}
\end{example}
%
...
proof for a familiar theorem, namely the cyclicity of the trace, which we shall now look at. 

\begin{example}
Consider two duals pairs 
\begin{align*}
...
The trace $\mathsf{tr}\left(f\circ g\right)$ is defined as the composition 
\[
...
.
\]
We can know show using the notation introduced above that $\mathsf{tr}\left(f\circ g\right)=\mathsf{tr}\left(g\circ f\right)$ as follows:
\begin{eqnarray*}
