...
%But what if one would like to add additional relations *** {\color{red} If every SMC is equivalent to a SSMC and those are equivalent to props, why do we need presentations of props? I understand that we need them to reason about structures in arbitrary props defined by generators and relations, while somehow props (without looking at their presentations) are only sufficient to reason about free props. HOW DOES THIS MAKE SENSE? why aren't props to enough to reason about props?}
...
% \begin{lem*}
%     Every PROP $\mc{P}$ admits a presentation of the form
%     \[
%         \mathfrak{F}\mc{R} \rightrightarrows \mathfrak{F}\mc{G} \rightarrow \mc{P}
%     \]
%     where $\mc{G}$ and $\mc{R}$ are signatures whose sets of objects are the same as that of $\mc{P}$, $\mathfrak{F}$ is the free PROP on a signature functor, and the two morphisms of PROPs acting as the identity on objects ({\color{red} clarify?}).
% \end{lem*}
