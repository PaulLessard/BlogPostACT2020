...
\begin{remark}
Taking a hint from the bicategorical playbook can give us a more geometric picture for the difference between an indecomposeable tuple and a decomposeable one is. Since every symmetric monoidal category is also a bicategory with a single object, a typed term, usually a $1$-cell, also corresponds to $2$-cells between (directed) loops. Thinking in this vein we may think of an indecomposeable tuple $(x_{(1)},x_{(2)},\dots,x_{(n)}):\overrightarrow{A}$ as an $n$-sphere in $\overrightarrow{A}$ whereas a decomposeable one, say $(x,y,\dots,z):\overrightarrow{A}$, corresponds to a torus (of some dimension) in  $\overrightarrow{A}$.
\end{remark}
...
Having sketched the basics of Shulman's \textsf{PTT} and how a presentation for a \textsf{PROP} specifies the rules of \textsf{PTT} for that \textsf{PROP} we may attend to an important and illuminating example. We will devolop the practical type theory of the free dual pair.
...
\]
where the first rule is an instance of the identity rule with premised generators $\coev$ and the braiding $(132)$, and the second rule is an instance of the identity rule with the typing judgement $x:D\vdash\left(\coev_{\left(1\right)},\coev_{\left(2\right)},x\right):D$ the consequent of the first rule and the additional hyopthesis being the generator \textsf{ev}.
\end{example}
%
