...
Many well-known type theories, e.g. Martin-L\"{o}f dependent type theory or linear type theory, have as their starting points particular forms of reasoning that we wish to treat syntactically (here constructive mathematical reasoning and resource dependent computation respectively). In a sense, it is only a happy accident that these type theories then provide means to reason \emph{about} locally cartesian closed categories or $\star$-autonomous categories.
...
But which properties of type theory make it practical? Shulman, sythesizing various observations, settles on a few basic tenets to guide the invention of the syntax (and it's interpretation into symmetric monoidal categories); we reduce these further to the conditions that the type theory be: (1) intuitive, (2) concise, and (3) effective.
...
\paragraph{Concise} Second, when working with a specific category, one tends, whether explicitly acknowledged or not, to work with a presentation (of that catgery) in terms of generators and relations, e.g. $\bigtriangleup$ as generated by face and degeneracy maps subject to the simplicial identities. As such, our type theory should not be directly specified in terms of a symmetric monoidal category, but rather in terms of a concise presentation of it. 
...
\begin{definition} A \textsf{PROP}, $\mathfrak{P}$, consists of a set $\mathbf{P}$ of generating objects and a symmetric strict monoidal category whose underlying monoid of objects is the free monoid on the set $\mathbf{P}$.
\end{definition}
...
It is from these presentations of $\mathsf{PROP}s$, the triples $(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)$, that we'll build type theories $\mathsf{PTT}_{(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)}$ for the symmetric monoidal categories $F(\mathbf{R},\mathbf{X}_1,\mathbf{X}_0)$. Indeed, reading $\rightsquigarrow$ as ``gives rise to'', we have
    \begin{eqnarray*}
      \mathbf{Generators}
        &
            &
...
\[
    ...
    (f:\overrightarrow{A} \rightarrow \overrightarrow{F}) \in \mathbf{X}_1, \dots , (g:\overrightarrow{B} \rightarrow \overrightarrow{G}) \in \mathbf{X}_1 \\
...
are similarly derived from rules coming from the set $\mathbf{R}$. We'll be more explicit in the context of an example later on.
...
\begin{itemize}
\item the rule $$\vcenter{\vbox{\inferrule{\left(x:A\right)\in\Gamma}{\Gamma\vdash x\ \mathsf{term}}}}$$ is the usual variable rule (giving us the terms in which we'll be able to derive the typing judgements corresponding to identities);
\item the rule $$\vcenter{\vbox{\inferrule{1\leq k\leq2}{\coev_{\left(k\right)} \ \mathsf{term}}
}}$$
which gives us terms in which we'll be able to derive co-evaluation as a typing judgement; and
\item the rule $$\vcenter{\vbox{\prftree{\Gamma\vdash m\ \mathsf{term}}{\Gamma\vdash n\ \mathsf{term}}{\Gamma\vdash\evmap\left(m,n\right)\ \mathsf{term}}}}$$ gives us the terms with which we'll be able to derive evaluation as a typing judgement.
\end{itemize}

We have two (streamlined) rules for typing judgements:
\begin{itemize}
    \item the rule
        \[
            \inferrule{
        ...
        \item the rule
            ...
            to the tensoring of:
            \begin{itemize}
                \item some number of generating constants, here $f,\dots,g$, with can only be some number of copies of $\coev$; and
                \item the identity on some object $\overrightarrow{A}$
            \end{itemize}
\end{itemize} 
\begin{remark} In recording these rules for the typing judgement we've obscured the technical consideration of ``activeness'', which is a device introduced to rigidify the type theory into one with unique derivations of judgements, and some syntactic sugar which acts like formal variables for the unit type.
\end{remark}
...
The typing judgement corresponding to that first morphism admits the following derivation 
\[
\prftree{
    \prftree{
...
\[
\prftree{\msf{triangle}\in \mathbf{R}(the things)}{M:D\vdash\left(\coev_{\left(1\right)}|\evmap\left(\coev_{\left(2\right)},M\right)\right)=M:D}
\]
and 
\[
\prftree{\msf{triangle}^\star\in\mathbf{R}(the things)}{N:D^{*}\vdash\left(\coev_{\left(2\right)}|\evmap\left(N,\coev_{\left(1\right)}\right)\right)=N:D^{*}}
\]
...
\begin{lem}
[Cyclicity of the trace] Let $\left(\mathfrak{C},\left(\_,\_\right),\left(\right)\right)$
